	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($, undefined) {
			...
			$.jstreeol3.defaults.visibility = {
				/**
				 * a boolean indicating if visibility should be visible (can be
				 * changed at a later time using `show_visibility()` and
				 * `hide_visibility`). Defaults to `true`.
				 * 
				...
				visible : true,
				/**
				 * a boolean indicating if visibility should cascade down and
			...
			$.jstreeol3.plugins.visibility = function(options, parent) {
																	...
																	for (i = 0, j = dpc.length; i < j; i++) {
																		if (m[dpc[i]].state['vanished']) {
																			for (k = 0, l = m[dpc[i]].children_d.length; k < l; k++) {
																				...
																				var layer = this
																						.get_LayerById(m[m[dpc[i]].children_d[k]].id);
																				var git;
																				if (layer) {
																					git = layer.get("git");
																				}
																				if (layer instanceof ol.layer.Base && git) {
																					if (git["fake"] === "child") {
																						...
																						if (parent instanceof ol.layer.Tile && git) {
																							var psource = parent.getSource();
																							var pparams = parent.getSource().getParams();
																							var childrenNames = pparams["LAYERS"]
																									.split(",");
																							childrenNames.splice(childrenNames.indexOf(this
																									.get_LayerById(obj.id).get("id")), 1);
																							pparams["LAYERS"] = childrenNames.toString();
																		...
																		if (c === j) {
																			...
																			if (tmp && tmp.length) {
																				tmp.attr('aria-selected', true).children(
																						'.jstreeol3-anchor').addClass('jstreeol3-checked');
																			}
																		} else {
																			break;
																		}
										...
										$
												.proxy(
														function(e, data) {
																		...
																		if (git["fake"] === "child") {
																			...
																			if (parent instanceof ol.layer.Tile && git) {
																				...
																				children.sort(function(a, b) {
																					return a.getZIndex() < b.getZIndex() ? -1 : a
																							.getZIndex() > b.getZIndex() ? 1 : 0;
																				});
															...
															// separately?)
															if (s.indexOf('down') !== -1 && dom.length) {
																dom.find('.jstreeol3-anchor').removeClass('jstreeol3-checked').parent()
													...
													if (layer instanceof ol.layer.Base && git) {
														if (git["fake"] === "child") {
															...
															if (parent instanceof ol.layer.Tile && git) {
																...
																var pparams = parent.getSource().getParams();
																var childrenNames = pparams["LAYERS"].split(",");
																childrenNames.splice(childrenNames.indexOf(this.get_LayerById(obj.id).get(
																		"id")), 1);
																pparams["LAYERS"] = childrenNames.toString();
																psource.updateParams(pparams);
																console.log(childrenNames);
							...
							function() {
								...
								if (!tmp.state.loaded) {
									...
									for (i = 0, j = tmp.children_d.length; i < j; i++) {
										...
										if (!tmp2.state.loaded && tmp2.original && tmp2.original.state
												&& tmp2.original.state.undeterminedVisibility
				...
				this.enable_visibility = function(obj) {
					...
					if (obj.state.visibility_disabled) {
						...
						if (dom && dom.length) {
							dom.children('.jstreeol3-anchor').children('.jstreeol3-visibility')
									.removeClass('jstreeol3-visibility-disabled');
						}
						/**
				...
				/**
				 * check a node (only if tie_selection in visibility settings is
				...
				this.check_node = function(obj, e) {
					...
					if (!obj.state.vanished) {
						 ...
						 * tie_selection in visibility settings is false)
						 * 
						 * @event
				...
				/**
				 * uncheck a node (only if tie_selection in visibility settings
				 ...
				 * visibility settings is false, otherwise select_all will be
				 * called internally)
				 * 
				 * @name check_all()
				...
				/**
				 * uncheck all checked nodes (only if tie_selection in
				 * visibility settings is false, otherwise deselect_all will be
				 * called internally)
				 * 
				 * @name uncheck_all()
						...
						if (tmp && tmp.state.loaded) {
							for (k = 0, l = tmp.children_d.length; k < l; k++) {
								if (this._model.data[tmp.children_d[k]].state.vanished) {
									c = true;
									this._data.visibility.vanished = $.vakata.array_remove_item(this._data.visibility.vanished,
											tmp.children_d[k]);
								}
							}
						}
									...
									if (git) {
										if (inst._data.layerproperties.properties.getFeatureRecord().isEditing(layer)) {
											...
											if (obj.type === "ImageTile") {
												...
												inst._data.layerproperties.properties.getImageTileInfo("geoserver/getGeoLayerInfoList.ajax", layer);
											}
