...
(function($, undefined) {
	 ...
	 * options: jslint: loopfunc: true, browser: true, ass: true, bitwise: true,
	 * continue: true, nomen: true, plusplus: true, regexp: true, unparam: true,
	 * todo: true, white: true
	 */
	/* jshint -W083 */
	...
	$.jstree = {
			...
			defaults : {
				/**
				 * configure which plugins will be active on an instance. Should
				 * be an array of strings, where each element is a plugin name.
				 * The default is `[]`
				 * 
	 ...
	 *            el the element to create the instance on, can be jQuery
	 *            extended or a selector
	 * @param {Object}
	 ...
	 * 
	 * __Examples__ // provided a container with an ID of "tree", and a nested
	 * node with an ID of "branch" // all of there will return the same instance
	 ...
	 * arguments passed to the function. If the function returns a value it will
	 * be returned (chaining could break depending on function).
	 * 
	 ...
	 * __Examples__
	 * 
	 * $('#tree1').jstree(); // creates an instance $('#tree2').jstree({ plugins : []
	 * }); // create an instance with some options
	 * $('#tree1').jstree('open_node', '#branch_1'); // call a method on an
	 * existing instance, passing additional arguments $('#tree2').jstree(); //
	 * get an existing instance (or create an instance)
	 * $('#tree2').jstree(true); // get an existing instance (will not create
	 * new instance) $('#branch_1').jstree().select_node('#branch_1'); // get an
	...
	$.jstree.defaults.core = {
			 ...
			 * data configuration
			 * 
			 * If left as `false` the HTML inside the jstree container element
			 * is used to populate the tree (that should be an unordered list
			 * with list items).
			 * 
			 * You can also pass in a HTML string or a JSON array here.
			 * 
			 * It is possible to pass in a standard jQuery-like AJAX config and
			 * jstree will automatically determine if the response is JSON or
			 * HTML and use that to populate the tree. In addition to the
			 * standard jQuery ajax options here you can suppy functions for
			 * `data` and `url`, the functions will be run in the current
			 * instance's scope and a param will be passed indicating which node
			 * is being loaded, the return value of those functions will be
			 * used.
			 * 
			 ...
			 * 'text' : 'Root node with options', 'state' : { 'opened' : true,
			 * 'selected' : true }, 'children' : [ { 'text' : 'Child 1' },
			 * 'Child 2'] } ] } }); // function $('#tree').jstree({ 'core' : {
			 * 'data' : function (obj, callback) { callback.call(this, ['Root
			 * 1', 'Root 2']); } });
			 * 
			 ...
			 * needed string and should return the replacement. If left as
			 * `false` no replacement is made.
			 * 
			 * __Examples__
			 * 
			...
			strings : false,
			/**
			 * determines what happens when a user tries to modify the structure
			 * of the tree If left as `false` all operations like create,
			 * rename, delete, move or copy are prevented. You can set this to
			 * `true` to allow all interactions or use a function to have better
			 * control.
			 * 
			 * __Examples__
			 * 
			 ...
			 * node_position is filled with the new node name return operation
			 ...
			 * instance's scope when something goes wrong (operation prevented,
			 * ajax failed, etc)
			 * 
			...
			themes : {
				 ...
				 * have manually included the theme CSS (recommended). You can
				 * set this to `true` too which will try to autoload the theme.
				 * 
				 * @name $.jstree.defaults.core.themes.url
				 */
				url : false,
				/**
				 * the location of all jstree themes - only used if `url` is set
				 * to `true`
				 * 
				 ...
				 * should kick in on smaller screens (if the theme supports it).
				 * Defaults to `false`.
				 * 
			 ...
			 * requests. Workers are however about 30% slower. Defaults to
			 * `true`
			 * 
	...
	$.jstree.core.prototype = {
			 ...
			 * @name destroy()
			 * @param {Boolean}
			 *            keep_html if not set to `true` the container will be
			 *            emptied, otherwise the current DOM elements will be
			 *            kept intact
			 */
			...
			bind : function() {
				...
				this.element.on("dblclick.jstree", function(e) {
					 ...
					 * this.get_selected() : o; this.delete_node(o); } break;
					 * 
					 */
					}
			...
			/**
			 * get the path to a node, either consisting of node texts, or of
			 * node IDs, optionally glued together (otherwise an array)
			 * 
			 * @name get_path(obj [, glue, ids])
			 * @param {mixed}
			 *            obj the node
			 * @param {String}
			 *            glue if you want the path as a string - pass the glue
			 *            here (for example '/'), if a falsy value is supplied
			 *            here, an array is returned
			 * @param {Boolean}
			 *            ids if set to true build the path using ID, otherwise
			 *            node text is used
			 * @return {mixed}
			 */
			...
			/**
			 * get a jQuery collection of all the children of a node (node must
			 * be rendered)
			 * 
			 ...
			 *            complete, the function is executed in the instance's
			 *            scope and receives two arguments - the node and a
			 *            boolean status
			 * @return {Boolean}
			...
			/**
			 * load an array of nodes (will also load unavailable nodes as soon
			 * as the appear in the structure). Used internally.
			 * 
			 * @private
			 ...
			 *            callback a function to be executed once loading is
			 *            complete, the function is executed in the instance's
			 *            scope and receives one argument - the array passed to
			 *            _load_nodes
			 */
			 ...
			 *            nodes in the tree
			 * @param {function}
			 *            callback a function to be executed once loading all
			 ...
			 *            callback a function to be executed once loading is
			 *            complete, the function is executed in the instance's
			 *            scope and receives one argument - a boolean status
			 * @return {Boolean}
			 */
			 ...
			 */
			_redraw : function() {
				var nodes = this._model.force_full_redraw ? this._model.data[$.jstree.root].children.concat([]) : this._model.changed.concat([]), f = document
			...
			/**
			 * opens a node, revaling its children. If the node is not loaded it
			 ...
			 * @param {Function}
			 *            callback a function to execute once the node is opened
			 * @param {Number}
			 *            animation the animation duration in milliseconds when
			 *            opening the node (overrides the `core.animation`
			 *            setting). Use `false` for no animation.
				...
				if (!this.is_loaded(obj)) {
					...
					if (d.length) {
						...
						if (!animation) {
							...
							d.children(".jstree-children").css("display", "none").end().removeClass("jstree-closed").addClass("jstree-open")
							.attr("aria-expanded", true).children(".jstree-children").stop(true, true).slideDown(animation, function() {
								this.style.display = "";
								if (t.element) {
					...
					if (!d.length) {
						/**
						 * triggered when a node is about to be opened (if the
						 * node is supposed to be in the DOM, it will be, but it
					...
					/**
					 * triggered when a node is opened (if there is an animation
			 ...
			 * @param {Number}
			 *            animation the animation duration in milliseconds when
			 *            closing the node (overrides the `core.animation`
			 *            setting). Use `false` for no animation.
			...
			close_node : function(obj, animation) {
				...
				if (!d.length) {
					...
					if (!animation) {
						...
						d.children(".jstree-children").attr("style", "display:block !important").end().removeClass("jstree-open").addClass(
						"jstree-closed").attr("aria-expanded", false).children(".jstree-children").stop(true, true).slideUp(animation,
								function() {
							this.style.display = "";
			...
			/**
			 * toggles a node - closing it if it is open, opening it if it is
			 * closed
			 * 
			 ...
			 * @param {Number}
			 *            animation the animation duration in milliseconds when
			 *            opening the nodes, the default is no animation
			 * @param {jQuery}
			 ...
			 * @param {mixed}
			 *            obj the node to close recursively, omit to close all
			 *            nodes in the tree
			 * @param {Number}
			 *            animation the animation duration in milliseconds when
			 *            closing the nodes, the default is no animation
			 ...
			 * @param {Boolean}
			 *            skip_redraw internal parameter controlling if redraw
			 *            is called
			...
			activate_node : function(obj, e) {
				...
				this._data.core.last_clicked = this._data.core.last_clicked && this._data.core.last_clicked.id !== undefined ? this
						 ...
						 *            triggered the call (may be an empty
						 *            object)
						 */
			...
			/**
			 * applies the hover state on a node, called when a node is hovered
			 * by the user. Used internally.
			 * 
			 * @private
			 * @name hover_node(obj)
			 * @param {mixed}
			 ...
			 * @param {Boolean}
			 *            supress_event if set to `true` the `changed.jstree`
			 *            event won't be triggered
			 * @param {Boolean}
			 *            prevent_open if set to `true` parents of the selected
			 *            node won't be opened
			 ...
			 * @param {mixed}
			 *            obj an array can be used to deselect multiple nodes
			 * @param {Boolean}
			 *            supress_event if set to `true` the `changed.jstree`
			 *            event won't be triggered
			 ...
			 * @param {mixed}
			 *            full if set to `true` the returned array will consist
			 *            of the full node objects, otherwise - only IDs will be
			 *            returned
			 * @return {Array}
			 */
			...
			/**
			 * get an array of all top level selected nodes (ignoring children
			 * of selected nodes)
			 * 
			 ...
			 * @private
			 * @param {Object}
			 *            state the state to restore. Keep in mind this object
			 ...
			 *            reapplied, if set to a function (receiving the current
			 *            state as argument) the result of that function will be
			 *            used as state
			 ...
			 * nodes inside that node are reloaded with calls to `load_node`.
			 * 
			...
			refresh_node : function(obj) {
				...
				this._load_nodes(to_load, $.proxy(function(nodes) {
					 ...
					 * @param {Array}
					 *            nodes - an array of the IDs of the nodes that
					 *            were reloaded
					 */
			 ...
			 * @param {mixed}
			 *            obj the node, you can pass an array to set the text on
			 *            multiple nodes
			 * @param {String}
			 ...
			 * @param {mixed}
			 *            par the parent node (to create a root node use either
			 *            "#" (string) or `null`)
			 * @param {mixed}
			 *            node the data for the new node (a valid JSON object,
			 *            or a simple string with the name)
			 * @param {mixed}
			 *            pos the index at which to insert the node, "first" and
			 *            "last" are also supported, default is "last"
			 * @param {Function}
			 *            callback a function to be called once the node is
			 *            created
			 * @param {Boolean}
			 *            is_loaded internal argument indicating if the parent
			 *            node was succesfully loaded
			...
			create_node : function(par, node, pos, callback, is_loaded) {
				 ...
				 * @param {Number}
				 *            position the position of the new node among the
				 *            parent's children
				 */
			 ...
			 * @param {mixed}
			 *            obj the node, you can pass an array to rename multiple
			 *            nodes to the same name
			 * @param {String}
			 *            val the new text value
			 * @return {Boolean}
			 ...
			 * @param {mixed}
			 *            obj the node, you can pass an array to delete multiple
			 *            nodes
			 * @return {Boolean}
			 ...
			 *            the new name
			 * @param {mixed}
			 *            more some various additional information, for example
			 *            if a "move_node" operations is triggered by DND this
			 ...
			 * @param {mixed}
			 *            obj the node to move, pass an array to move multiple
			 *            nodes
			 * @param {mixed}
			 *            par the new parent
			 * @param {mixed}
			 *            pos the position to insert at (besides integer values,
			 *            "first" and "last" are supported, as well as "before"
			 *            and "after"), defaults to integer `0`
			 * @param {function}
			 *            callback a function to call once the move is
			 *            completed, receives 3 arguments - the node, the new
			 *            parent and the position
			 * @param {Boolean}
			 *            is_loaded internal parameter indicating if the parent
			 *            node has been loaded
			 * @param {Boolean}
			 *            skip_redraw internal parameter indicating if the tree
			 *            should be redrawn
			 * @param {Boolean}
			...
			move_node : function(obj, par, pos, callback, is_loaded, skip_redraw, origin) {
				...
				old_pos = old_ins && old_ins._id && old_par && old_ins._model.data[old_par] && old_ins._model.data[old_par].children ? $.inArray(
						 ...
						 *            parent the parent's ID
						 * @param {Number}
						 *            position the position of the node among
						 ...
						 * @param {Boolean}
						 *            is_multi do the node and new parent belong
						 *            to different instances
						 * @param {jsTree}
						 *            old_instance the instance the node came
						 *            from
						 * @param {jsTree}
						 *            new_instance the instance of the new
						 *            parent
						 */
			 ...
			 * @param {mixed}
			 *            obj the node to copy, pass an array to copy multiple
			 *            nodes
			 * @param {mixed}
			 *            par the new parent
			 * @param {mixed}
			 ...
			 *         "move_node"), `node` (an array of objects) and `inst`
			 *         (the instance)
			 */
			get_buffer : function() {
				return {
			 ...
			 *            "first" and "last" are supported), defaults to integer
			 *            `0`
			...
			paste : function(obj, pos) {
				...
				if (this[ccp_mode](ccp_node, obj, pos, false, false, false, ccp_inst)) {
					 ...
					 * @param {String}
					 *            mode the performed operation - "copy_node" or
					 *            "move_node"
					 */
					this.trigger('paste', {
						"parent" : obj.id,
			 ...
			 *            obj
			 * @param {String}
			 *            default_text the text to populate the input with (if
			 *            omitted or set to a non-string value the node's text
			 *            value is used)
			 * @param {Function}
			 *            callback a function to be called once the text box is
			 *            blurred, it is called in the instance's scope and
			 *            receives the node, a status parameter (true if the
			 *            rename is successful, false otherwise) and a boolean
			 *            indicating if the user cancelled the edit. You can
			 *            access the node's title using .text
			 */
			 ...
			 * @param {mixed}
			 *            theme_url the location of the CSS file for this theme.
			 *            Omit or set to `false` if you manually included the
			 *            file. Set to `true` to autoload from the
			 ...
			 *            variant_name the variant to apply (if `false` is used
			 *            the current variant is removed)
			 */
			 ...
			 *            className, if using an image that is in the current
			 *            directory use a `./` prefix, otherwise it will be
			 *            detected as a class
			 */
	...
	$.jstree.plugins.changed = function(options, parent) {
		...
		this.trigger = function(ev, data) {
			 ...
			 *            selected the current selection
			 * @param {Object}
			 *            changed an object containing two properties `selected`
			 *            and `deselected` - both arrays of node IDs, which were
			 *            selected or deselected since the last changed event
			 * @param {Object}
			 *            event the event (if any) that triggered this changed
			 *            event
			 * @plugin changed
			 */
	 ...
	 * selection much easier. It also supports tri-state behavior, meaning that
	 * if a node has a few of its children checked it will be rendered as
	 * undetermined, and state will be propagated up.
	 */
	...
	$.jstree.defaults.checkbox = {
			...
			visible : true,
			/**
			 * a boolean indicating if checkboxes should cascade down and have
			 * an undetermined state. Defaults to `true`.
			 * 
			...
			three_state : true,
			/**
			 * a boolean indicating if clicking anywhere on the node should act
			 * as clicking on the checkbox. Defaults to `true`.
			 * 
			...
			whole_node : true,
			/**
			 * a boolean indicating if the selected style of a node should be
			 * kept, or removed. Defaults to `true`.
			 * 
			 ...
			 * 'undetermined' is in the string - undetermined nodes will be
			 * used. If `three_state` is set to `true` this setting is
			 ...
			 * selection or to an internal array maintained by the checkbox
			 * plugin. Defaults to `true`, only set to `false` if you know
	...
	$.jstree.plugins.checkbox = function(options, parent) {
												...
												.proxy(
														function(e, data) {
															...
															if (s.indexOf('down') !== -1 && dom.length) {
																dom.find('.jstree-anchor').addClass(t ? 'jstree-clicked' : 'jstree-checked').parent()
		...
		/**
		 * uncheck all checked nodes (only if tie_selection in checkbox settings
		 ...
		 * @param {mixed}
		 *            full if set to `true` the returned array will consist of
		 *            the full node objects, otherwise - only IDs will be
		 *            returned
		 * @return {Array}
		 * @plugin checkbox
		 */
	...
	$.jstree.defaults.geoserver = {
			 ...
			 * 레이어를 요청할 도메인 주소
			 * 
			 * @name $.jstree.defaults.geoserver.server_addr
			 * @plugin geoserver
			 */
			 ...
			 * wms 파라미터
			 * 
			 * @name $.jstree.defaults.geoserver.param_wms
			 * @plugin geoserver
			 */
			param_wms : {
				"LAYER" : undefined,
					...
					if (Array.isArray(data)) {
						for (var i = 0; i < data.length; i++) {
							...
							for (var j = 0; j < data[i].publishedList.names.length; j++) {
								var layer = new ol.layer.Base();
								var id = data[i].publishedList.names[j];
								var name = id.substring(id.split("_", 3).join("_").length, id.split("_", 4).join("_").length);
								var gchild = {
										"validation" : false,
										"editable" : true,
										"fake" : "child"
	...
	$.jstree.defaults.contextmenu = {
			 ...
			 * node. Defaults to `true`, otherwise the mouse coordinates are
			 * used.
			 * 
			 ...
			 * callback function and calls the callback function with an object
			 * of actions available for that node (you can also return the items
			 * too).
			 * 
			 * Each action consists of a key (a unique name) and a value which
			 * is an object with the following properties (only label and action
			 * are required). Once a menu item is activated the `action`
			 * function will be invoked with an object containing the following
			 * keys: item - the contextmenu item definition as seen below,
			 * reference - the DOM node that was used (the tree node), element -
			 * the contextmenu DOM element, position - an object with x/y
			 * properties indicating the position of the menu. *
			 * `separator_before` - a boolean indicating if there should be a
			 * separator before this item * `separator_after` - a boolean
			 * indicating if there should be a separator after this item *
			 * `_disabled` - a boolean indicating if this action should be
			 * disabled * `label` - a string - the name of the action (could be
			 * a function returning a string) * `title` - a string - an optional
			 * tooltip for the item * `action` - a function to be executed if
			 * this item is chosen, the function will receive * `icon` - a
			 * string, can be a path to an icon or a className, if using an
			 * image that is in the current directory use a `./` prefix,
			 * otherwise it will be detected as a class * `shortcut` - keyCode
			 * which will trigger the action if the menu is open (for example
			 * `113` for rename, which equals F2) * `shortcut_label` - shortcut
			 * label (like for example `F2` for rename) * `submenu` - an object
			 * with the same structure as $.jstree.defaults.contextmenu.items
			 * which can be used to create a submenu - each key will be rendered
...
//										inst.import_fake_group(obj);
	...
	$.jstree.plugins.contextmenu = function(options, parent) {
		this.bind = function() {
			 ...
			 * document.body)) { var el = null, tm = null; this.element
			 * .on("touchstart", ".jstree-anchor", function (e) { el =
			 * e.currentTarget; tm = +new Date(); $(document).one("touchend",
			 * function (e) { e.target =
			 * document.elementFromPoint(e.originalEvent.targetTouches[0].pageX -
			 * window.pageXOffset, e.originalEvent.targetTouches[0].pageY -
			 * window.pageYOffset); e.currentTarget = e.target; tm = ((+(new
			 * Date())) - tm); if(e.target === el && tm > 600 && tm < 1000) {
			 * e.preventDefault(); $(el).trigger('contextmenu', e); } el = null;
			 * tm = null; }); }); }
			 */
		 ...
		 *            obj the node
		 * @param {Number}
		 *            x the x-coordinate relative to the document to show the
		 *            menu at
		 * @param {Number}
		 *            y the y-coordinate relative to the document to show the
		 *            menu at
		 * @param {Object}
		 *            e the event if available that triggered the contextmenu
		 * @plugin contextmenu
	...
	$.jstree.defaults.dnd = {
			/**
			 * a boolean indicating if a copy should be possible while dragging
			 * (by pressint the meta key or Ctrl). Defaults to `true`.
			 * 
			 ...
			 * invoked in the tree's scope and receives the nodes about to be
			 * dragged as an argument (array) and the event that started the
			 * drag - return `false` to prevent dragging
			 * 
			...
			is_draggable : true,
			/**
			 * a boolean indicating if checks should constantly be made while
			 * the user is dragging the node (as opposed to checking only on
			 ...
			 * controls if all selected nodes are dragged or only the single
			 * node, default is `true`, which means all selected nodes are
			 * dragged when the drag is started on a selected node
			 * 
			...
			drag_selection : true,
			/**
			 * controls whether dnd works on touch devices. If left as boolean
			 * true dnd will work the same as in desktop browsers, which in some
			 * cases may impair scrolling. If set to boolean false dnd will not
			 * work on touch devices. There is a special third option - string
			 * "selected" which means only selected nodes can be dragged on
			 * touch devices.
			 * 
			...
			touch : true,
			/**
			 * controls whether items can be dropped anywhere on the node, not
			 * just on the anchor, by default only the node anchor is a valid
			 * drop target. Works best with the wholerow plugin. If enabled on
			 * mobile depending on the interface it might be hard for the user
			 * to cancel the drop, since the whole tree container will be a
			 * valid drop target.
			 * 
			 ...
			 * scrolling on mobile depending on the interface - in that case set
			 * the touch option to "selected".
			 * 
											...
											function(e) {
												...
												if (e.type === "touchstart"
													&& (!this.settings.dnd.touch || (this.settings.dnd.touch === 'selected' && !$(
															e.currentTarget).closest('.jstree-node').children('.jstree-anchor').hasClass(
															'jstree-clicked')))) {
													return true;
												}
												...
												if (obj
													...
													if (this.settings.dnd.use_html5) {
														...
														return $.vakata.dnd.start(e, drg, '<div id="jstree-dnd" class="jstree-'
																+ this.get_theme() + ' jstree-' + this.get_theme() + '-'
				...
				function(e, data) {
					...
					if (ins && ins._data && ins._data.dnd) {
						...
						data.helper.children().attr(
								...
								+ (ins.settings.core.themes.responsive ? ' jstree-dnd-responsive' : '')).find('.jstree-copy')
								.first()[is_copy ? 'show' : 'hide']();
						...
						if ((data.event.target === ins.element[0] || data.event.target === ins.get_container_ul()[0])
										...
										(data.data.origin
												&& (data.data.origin.settings.dnd.always_copy || (data.data.origin.settings.dnd.copy && (data.event.metaKey || data.event.ctrlKey))) ? "copy_node"
														...
														(data.data.origin && data.data.origin !== ins ? data.data.origin
																.get_node(data.data.nodes[t1]) : data.data.nodes[t1]), $.jstree.root,
																'last', {
											'dnd' : true,
							...
							'.jstree-anchor') : $(data.event.target).closest('.jstree-anchor');
								...
								.each(
										...
										function(j, v) {
											...
											if (ok) {
												...
												data.helper.find('.jstree-icon').first().removeClass('jstree-er').addClass(
												'jstree-ok');
												if (data.event.originalEvent && data.event.originalEvent.dataTransfer) {
				...
				drag : function(e) {
					...
					if (!vakata_dnd.is_drag) {
						if (Math.abs(e.pageX - vakata_dnd.init_x) > (vakata_dnd.is_touch ? $.vakata.dnd.settings.threshold_touch
							 ...
							 *            mouse
							 * @param {Object}
							 *            event the event that caused the start
							 *            (probably mousemove)
							 */
							$.vakata.dnd._trigger("start", e);
						} else {
					 ...
					 *            helper the helper shown next to the mouse
					 * @param {Object}
					 *            event the event that caused this to trigger
	 ...
	 * 
	 * It is possible to set this to a standard jQuery-like AJAX config. In
	 * addition to the standard jQuery ajax options here you can supply
	 ...
	 * of that node as an array. { "id1" : [{ "text" : "Child of ID1", "id" :
	 * "c1" }, { "text" : "Another child of ID1", "id" : "c2" }], "id2" : [{
	...
	$.jstree.defaults.search = {
			/**
			 * a jQuery-like AJAX config, which jstree uses if a server should
			 ...
			 * search is limited to a node id. The expected result is a JSON
			 * array with nodes that need to be opened so that matching nodes
			 * will be revealed. Leave this setting as `false` to not query the
			 * server. You can also set this to a function, which will be
			 * invoked in the instance's scope and receive 3 parameters - the
			 * search string, the callback to call with the array of nodes to
			 * load, and the optional node ID to limit the search to
			 * 
			 ...
			 * match `child node 3`). Default is `false`.
			 * 
			 ...
			 * only matching nodes (keep in mind this can be a heavy on large
			 * trees in old browsers). This setting can be changed at runtime
			...
			/**
			 * Indicates if all nodes opened to reveal the search result, should
			 * be closed when the search is cleared or a new search is
			 ...
			 * with two arguments - search string and node (where node will be
			 * every node in the structure, so use with caution). If the
			 * function returns a truthy value the node will be considered a
			 * match (it might not be displayed if search_only_leaves is set to
			 * true and the node is not a leaf). Default is `false`.
			 * 
	...
	$.jstree.plugins.search = function(options, parent) {
		 ...
		 * @param {Boolean}
		 *            skip_async if set to true server will not be queried even
		 *            if configured
		 * @param {Boolean}
		 *            show_only_matches if set to true only matching nodes will
		 *            be shown (keep in mind this can be very slow on large
		 *            trees or old browsers)
		 * @param {mixed}
		 *            inside an optional node to whose children to limit the
		 *            search
		 * @param {Boolean}
		 *            append if set to true the results of this search are
		 *            appended to the previous search
		 * @plugin search
		...
		this.search = function(str, skip_async, show_only_matches, inside, append, show_only_matches_children) {
			 ...
			 *            str the search string
			 * @param {Array}
			 *            res a collection of objects represeing the matching
			 *            nodes
			 * @plugin search
			 */
			this.trigger('search', {
		...
		/**
		 * used to clear the last search (removes classes and shows all nodes if
		 * filtering is on)
		 * 
		 * @name clear_search()
		 * @plugin search
		...
		this.clear_search = function() {
			 ...
			 * @param {jQuery}
			 *            nodes a jQuery collection of matching nodes (the
	...
	/**
	 * the settings function used to sort the nodes. It is executed in the
	 * tree's context, accepts two nodes as arguments and should return `1` or
	 * `-1`.
	 * 
	...
	$.jstree.plugins.sort = function(options, parent) {
		 ...
		 *            obj the node
		 * @param {Boolean}
		 *            deep if set to `true` nodes are sorted recursively.
		 * @plugin sort
		 * @trigger search.jstree
		 */
	...
	$.jstree.defaults.state = {
			/**
			 * A string for the key to use when saving the current tree (change
			 * if using multiple trees in your project). Defaults to `jstree`.
			 * 
			...
			/**
			 * Time in milliseconds after which the state will expire. Defaults
			 * to 'false' meaning - no expire.
			 * 
			...
			ttl : false,
			/**
			 * A function that will be executed prior to restoring state with
			 * one argument - the state object. Can be used to clear unwanted
			 * parts of the state.
			 * 
	 ...
	 * ### Types plugin
	 * 
	 * Makes it possible to add predefined types for groups of nodes, which make
	 * it possible to easily control nesting rules and icon for each group.
	 */

	/**
	 * An object storing all types as key value pairs, where the key is the type
	 * name and the value is an object that could contain following keys (all
	 * optional). * `max_children` the maximum number of immediate children this
	 * node type can have. Do not specify or set to `-1` for unlimited. *
	 ...
	 * `valid_children` an array of node type strings, that nodes of this type
	 * can have as children. Do not specify or set to `-1` for no limits. *
	 ...
	 * theme. * `li_attr` an object of values which will be used to add HTML
	 * attributes on the resulting LI DOM node (merged with the node's own data) *
	 * `a_attr` an object of values which will be used to add HTML attributes on
	 * the resulting A DOM node (merged with the node's own data)
	 * 
	 * There are two predefined types: * `#` represents the root of the tree,
	 * for example `max_children` would control the maximum number of root
	 * nodes. * `default` represents the default node - any settings here will
	 * be applied to all nodes that do not have a type specified.
	 * 
	...
	$.jstree.plugins.types = function(options, parent) {
		 ...
		 *            obj the node to find the rules for
		 * @param {Boolean}
		 *            rules if set to `true` instead of a string the settings
	...
	$.jstree.defaults.unique = {
			 ...
			 * conflicting name and the counter. The default will produce
			 * results like `New node (2)`.
			 * 
