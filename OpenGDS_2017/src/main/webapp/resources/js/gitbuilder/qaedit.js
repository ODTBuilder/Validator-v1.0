...
gitbuilder.ui.QAEdit = $.widget("gitbuilder.qaedit", {
	...
	_create : function() {
		...
		var td11 = $("<td>").append(this.leftbtn), td22 = $("<td>").append(this.centerbtn), td33 = $("<td>").append(
				this.rightbtn);
		...
		var tb = $("<table>").addClass("table").addClass("table-bordered").addClass("text-center").append(tr1).append(
				tr11).append(tr111).append(tr1111);
	...
	_import_fake_group : function(jstreeRefer, group, groupInfoURL, wmsURL, wfsURL, groupLayerName, lastMapSheet,
			errorLayerName, errorCallback, inputLayer, instance) {
		// // =======================================
		var that = this;
				...
				if (Array.isArray(data)) {
					for (var i = 0; i < data.length; i++) {
						...
						for (var j = 0; j < data[i].publishedList.names.length; j++) {
							...
							var name = id.substring((id.split("_", 3).join("_").length) + 1,
									id.split("_", 4).join("_").length);
							var gchild = {
								"validation" : false,
								"editable" : true,
								"fake" : "child"
	...
	loadChildrenLayer : function(jstreeRefer, layerInfoURL, wmsURL, wfsURL, group, lastSheet, nowSheet,
			childrenLayerName, parentLayer, errorLayerName, errorCallback, instance) {
		// var that = this;
		var arr = {
	...
	start : function() {
		...
		if (errorLayerName !== undefined && arr.geoLayerList.length > 0) {
			...
			// this.options.featureWMSURL, this.options.featureWFSURL, arr,
			...
			this._import_fake_group($(that.list).jstree(true), group, this.options.groupInfoURL,
					this.options.featureWMSURL, this.options.featureWFSURL, arr, arr.geoLayerList.length - 1,
					errorLayerName, this.loadErrorLayer, this.inputLayer, this);
		}
	},
	_create_navigator : function() {
		var that = this;
		var prevIcon = $("<span>").addClass("glyphicon").addClass("glyphicon-backward"), nextIcon = $("<span>")
				.addClass("glyphicon").addClass("glyphicon-forward");
		var btnPrev = $("<button>").addClass("gitbuilder-qaedit-navigator-prev").addClass("btn")
				.addClass("btn-default").append(prevIcon), btnNext = $("<button>").addClass(
				"gitbuilder-qaedit-navigator-next").addClass("btn").addClass("btn-default").append(nextIcon);
