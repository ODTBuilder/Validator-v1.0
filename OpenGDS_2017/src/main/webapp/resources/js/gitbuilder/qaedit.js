...
gitbuilder.ui.QAEdit = $.widget("gitbuilder.qaedit", {
	...
	_create : function() {
		...
		var tb = $("<table>").addClass("table").addClass("table-bordered").addClass("text-center").append(tr1).append(tr11).append(tr111).append(tr1111);
				...
				if (Array.isArray(data)) {
					...
					if (error !== undefined) {
						var defaultParameters = {
							...
							request : 'GetFeature',
							typeName : error,
							// typeName : that.options.user + ":" + error,
							// maxFeatures : 10000,
	...
	_create_navigator : function() {
		...
		var btnPrev = $("<button>").addClass("gitbuilder-qaedit-navigator-prev").addClass("btn").addClass("btn-default").append(prevIcon), btnNext = $("<button>")
				.addClass("gitbuilder-qaedit-navigator-next").addClass("btn").addClass("btn-default").append(nextIcon);
	...
	nextError : function() {
		if (this.count > 0 && this.count < this.features.length - 1) {
			this.count++;
		}
		this.showFeatureInfo(this.source.getFeatureById(this.lid + "." + this.count));
	},
