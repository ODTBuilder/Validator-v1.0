	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($, undefined) {
			...
			$.jstreeol3.core.prototype = {
				...
				setUniqueLayerId : function(layer, id) {
					...
					if (typeof layer.get("id") !== "string") {
						if (layer instanceof ol.layer.Group) {
							...
							var layers = layer.getLayers();
							for (var i = 0; i < layers.getLength(); i++) {
								if (typeof layers.item(i).get("id") !== "string") {
				...
				bind : function() {
					...
					this.element.on("dblclick.jstreeol3", function(e) {
						...
						case 35: // end
							...
							break;
						case 113: // f2 - safe to include - if check_callback
				...
				_createJSONFromLayer : function(layer, parentid) {
					...
					if (layer instanceof ol.layer.Vector) {
						...
						var layers = layer.getLayers();
						for (var i = 0; i < layers.getLength(); i++) {
							result = result.concat(that._createJSONFromLayer(layers.item(i), layer.get("id")));
						}
					}
					return result;
				},
							...
							"submenu" : {
								...
								"paste" : {
									...
									"_disabled" : function(data) {
										return !$.jstreeol3.reference(data.reference).can_paste();
									},
									"separator_after" : false,
									"label" : "Paste",
									"action" : function(data) {
			...
			$.jstreeol3.defaults.types = {
				"#" : {
					"valid_children" : [ "default", "Point", "LineString", "Polygon", "MultiPoint", "MultiLineString", "MultiPolygon", "Group",
							"Vector", "Raster", "ImageTile", "ValidationGroup" ]
				},
				// 편집도구에서 지원할 타입
				"Point" : {
					"icon" : "fa fa-circle",
					...
					this.element.on('ready.jstreeol3 set_state.jstreeol3', $.proxy(function() {
						...
						for (i = 0, j = data.selected.length; i < j; i++) {
							...
							$.proxy(function(e, data) {
								...
								this.get_node(data.node, true).children('.jstreeol3-wholerow')[e.type === "hover_node" ? "addClass" : "removeClass"]
