...
public class NGIFileLayerParser {
	 ...
	 * @param ngiReader
	 */
	public NGIFileLayerParser(BufferedReader ngiReader) {
	 ...
	 * @param ndaReader
	 */
	public NGIFileLayerParser(BufferedReader ngiReader, BufferedReader ndaReader) {
		this.ngiReader = ngiReader;
	 ...
	 * IOException @throws
	 */
	public DTNGILayerList parseQA20LayersWithAtt() throws IOException {

		DTNGILayerList layers = new DTNGILayerList();
	 ...
	 * IOException @throws
	 */
	private DTNGILayerList parseQA20LayerWithAtt() throws IOException {
		...
		for (String type : layerTypes) {
			...
			if (isEquals == false) {
				...
				DTNGILayer layer = new DTNGILayer(layerID_type, true);
	 ...
	 * IOException @throws
	 */
	public DTNGILayerList parseQA20Layers() throws IOException {
	 ...
	 * IOException @throws
	 */
	private DTNGILayerList parseQA20Layer() throws IOException {
		...
		for (String type : layerTypes) {
			...
			if (!isEquals) {
				String layerID_type = layerID + "_" + type;
				DTNGILayer layer = new DTNGILayer(layerID_type, false);
				layer.setLayerName(layerName + "_" + type);
				layer.setLayerType(type);
				layer.setNgiHeader(ngiHeader);
				layers.add(layer);
		...
		while (ngiLine != null) {
			if (ngiLine.equalsIgnoreCase("<DATA>")) {
				...
				while (!ngiLine.equalsIgnoreCase("<END>")) {
					String featureID = ngiLine;
					NGIFileFeatureParser featureParser = new NGIFileFeatureParser(ngiReader);
					DTNGIFeature feature = featureParser.parserDTFeature(featureID);
					String type = feature.getFeatureType();
					layers.getQA20Layer(layerID + "_" + type).add(feature);
					ngiReader = featureParser.getNgiReader();
