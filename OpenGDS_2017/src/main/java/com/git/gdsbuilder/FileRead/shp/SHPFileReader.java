			...
			if (validShx && validDbf) {
				...
				if (collection != null) {
					...
					if (geomType.startsWith("Multi")) {
							...
							if (geomNum == 1) {
								...
								for (int j = 0; j < properties.size(); j++) {
									...
									if (propertyName.equals("the_geom")) {
										...
										AttributeType type = (AttributeType) featureType.getType(propertyName);
										String valueType = type.getBinding().getSimpleName();
						...
						dtLayer.setFeatureType(newCollection.getSchema());
